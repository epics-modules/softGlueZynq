...
if [ $# -lt 2 ]; then
	printf "%s\n" "usage: copyIPForGit source_dir ip_repo_dir"
	printf "%s\n" "example: copyIPForGit ~/zynq/VivadoProjects/ip_repo/IP/gateDly ./ip"
	exit
fi
...
if [ ! -d ${ip_repo_dir}/$source_dirname ]; then
	echo "mkdir ${ip_repo_dir}/$source_dirname"
	mkdir ${ip_repo_dir}/$source_dirname
fi
cp $source_dir/component.xml ${ip_repo_dir}/$source_dirname

if [ ! -d ${ip_repo_dir}/$source_dirname/src ]; then
	echo "mkdir ${ip_repo_dir}/$source_dirname/src"
	mkdir ${ip_repo_dir}/$source_dirname/src
fi
cp $source_dir/src/* ${ip_repo_dir}/$source_dirname/src

if [ ! -d ${ip_repo_dir}/$source_dirname/xgui ]; then
	echo "mkdir ${ip_repo_dir}/$source_dirname/xgui"
	mkdir ${ip_repo_dir}/$source_dirname/xgui
fi
