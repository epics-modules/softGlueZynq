...
if [ $# -lt 2 ]; then
	printf "%s\n" "usage: copyConstraintsForGit source_dir ip_repo_dir"
	printf "%s\n" "example: copyConstraintsForGit ~/zynq/VivadoProjects/softGlue_3_TTL_7020_dma/softGlue_3_TTL_7020_dma.srcs/constrs_1/imports/src/ ./ip"
	exit
fi
...
if [ ! -d ${ip_repo_dir}/constraints ]; then
	echo "mkdir ${ip_repo_dir}/constraints"
	mkdir ${ip_repo_dir}/constraints
fi
